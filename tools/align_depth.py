...
def align_monodepth_with_colmap(
    ...
    for image in tqdm(images, "Aligning monodepth with colmap point cloud"):
        ...
        mono_depth = np.load(monodepth_path)
        mono_depth = cv2.resize(mono_depth, (360, 480)) # #############################

        mono_depth_sparse = cv2.remap(
            mono_depth,  # type: ignore
        ...
        ratio = colmap_depth / (mono_depth_sparse + 1e-8)
        lo, hi = np.percentile(ratio, (5,95))
        good = (ratio >= lo) & (ratio <= hi)
        colmap_depth, mono_depth_sparse = colmap_depth[good], mono_depth_sparse[good]
        a, b = np.polyfit(mono_depth_sparse, colmap_depth, 1)

        mono_disp_aligned = a * mono_depth + b
        print(((a * mono_depth_sparse + b) - colmap_depth).mean())
