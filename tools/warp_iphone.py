...
if __name__ == "__main__":
    ...
    for idx in valid_index:
        ...
        cv2.imwrite(f'iphone/sample/warp_{idx}.png', cv2.cvtColor(warp_img, cv2.COLOR_BGR2RGB))
        cv2.imwrite(f"iphone/sample/mask_{idx}.png", (255.0*valid_mask).astype(np.uint8))
        cv2.imwrite(f"iphone/sample/source_{idx}.png", cv2.cvtColor(source_img, cv2.COLOR_BGR2RGB))
        cv2.imwrite(f"iphone/sample/target_{idx}.png", cv2.cvtColor(target_img, cv2.COLOR_BGR2RGB))
