...
class Camera:
    ...
    @classmethod
    def fromcolmap(cls, filename, basic_cam_info):
        camera_dict = load_json(filename)

        # Fix old camera JSON.
        if "tangential" in camera_dict:
            camera_dict["tangential_distortion"] = camera_dict["tangential"]
        orientation = basic_cam_info.extrinsic[:3,:3]
        position = -orientation @ basic_cam_info.extrinsic[:3,3][:, np.newaxis][:,0]
        focal_length = basic_cam_info.intrinsic[0,0]
        principal_point = basic_cam_info.intrinsic[:2,-1]
        return cls(
...
class iPhoneDataset(Dataset):
    ...
    def _load_scene_cameras(self):
        self.cam_infos = util.get_colmap_camera_params(sparse_dir= osp.join(self.data_root, self.scene, "flow3/colmap/sparse"))

    def _load_video(self, _factor=2): # Just for camera 0
        image_folder_path = osp.join(self.data_root, self.scene, 'rgb', f'{_factor}x')
        frame_images = os.listdir(image_folder_path)
        frame_images = [image_name for image_name in frame_images if image_name.split('_')[0] == '0']
        frame_images = natsorted(frame_images)
        image_list = []
        for id in tqdm(range(len(frame_images)), 'Loading video...'):
            image_path = os.path.join(image_folder_path, frame_images[id])
            image = Image.open(image_path)
            image_list.append(np.array(image.convert('RGB')).astype(np.uint8))
    ...
    def load_depth(
        ...
        depth_path = osp.join(
            self.data_root, "depths", self.scene, 'aligned_depth_vda', f"{self._factor}x", frame_name + ".npy"
        )
        # depth_path = osp.join(
        #     self.data_root, self.scene, "flow3" , 'aligned_depth_anything_colmap', f"1x", frame_name + ".npy"
        # )
        depth = np.load(depth_path) * self.scale
        # depth = cv2.resize(depth, (360, 480)) * self.scale
    ...
    def load_camera(
        ...
        camera = (
            Camera.fromcolmap(
                ...
                self.cam_infos[camera_id][f"{frame_name}.png"]
            )
